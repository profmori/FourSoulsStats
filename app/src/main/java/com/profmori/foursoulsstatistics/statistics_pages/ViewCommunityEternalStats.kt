...
class ViewCommunityEternalStats : AppCompatActivity() {
    ...
    private fun createTable(playerRange: List<Float>, treasureRange: List<Float>){
        ...
        eternalTable.headerAdapter = charHeaderAdapter

        eternalTable.setHeaderBackgroundColor(ContextCompat.getColor(this, R.color.darker))
        eternalTable.setBackgroundColor(ContextCompat.getColor(this, R.color.lighter))
        // Sets the table backgrounds as tints

        eternalTable.setColumnComparator(0,
            NameComparator()
        )
        eternalTable.setColumnComparator(1,
            WinrateComparator()
        )
        eternalTable.setColumnComparator(2,
            SoulsComparator()
        )
        eternalTable.setColumnComparator(3,
        ...
        lifecycleScope.launch {
            ...
            eternals.forEach {
                // For every eternal
                if(!it.isNullOrBlank()) {
                    val newCharTable = StatsTable(it, 0.0, 0.0, 0, 0.0)
                    // Create a new row
                    newCharTable.setEternalData(it, selectedGames)
                    // Update the data
                    eternalData += arrayOf(newCharTable)
                    // Add it to the table
                }
            }

            val charDataAdapter = StatsTableDataAdapter(this@ViewCommunityEternalStats, fonts["body"]!!, eternalData)
            // Create the character data adapter
            eternalTable.dataAdapter = charDataAdapter
            // Attach it to the table
            eternalTable.sort(AdjustedSoulsComparator())
            eternalTable.sort(SoulsComparator())
            eternalTable.sort(WinrateComparator())
